...
Cayenne.prototype.publish = function(topic, payload) {
    if (this.client) {
        if (DEBUG) {
            console.log("PUB %s ==> %s", topic, payload);    
        }
...
Cayenne.prototype.handleCommand = function (req) {
    ...
    const cmd = payload[1];
    if (DEBUG) {
        console.log("RCV v1/%s/things/%s ==> %s", req.meta.username, req.meta.thingId, payload);    
    }

    this.publish(this.rootTopic + '/response', 'ok,'+seq);
    this.publish(this.rootTopic + '/data/' + req.meta.channel, cmd);

    this.emit("cmd" + req.meta.channel, req.meta.channel, cmd);
}
